...
# ============================================================
# V7 - Operational Historical Signals (NO LEAKAGE)
# - Uses only pre-flight + historical (shifted) aggregates
# - Keeps your best setup: CatBoost GPU + Platt + Prior-shift + Threshold constraint
# - Adds strong V7 signals:
#   * tail-number history (cascade effect)
#   * airport operational history (dep delay & cause rates)
#   * route operational history
#   * cause proxies (weather/system/late aircraft) as historical rates
# ============================================================
...
# Threshold constraint (you can tune)
...
# scale_pos_weight strategy (your best so far)
SPW_MULTIPLIER = 0.85

# CatBoost params (best from your V6.2 runs)
CB_PARAMS = dict(
    iterations=2000,
    learning_rate=0.06,
    depth=8,
...
def _rolling_sum_shifted(series: pd.Series, window: int, min_periods: int | None = None) -> pd.Series:
    if min_periods is None:
        min_periods = max(3, min(window, 20))
    else:
        min_periods = min(min_periods, window)
...
def add_rolling_mean_feature(df: pd.DataFrame, group_cols: list[str], value_col: str, out_col: str, windows=(5, 10, 30)) -> pd.DataFrame:
    """
    Rolling mean of a numeric column, shifted(1) to avoid leakage.
    """
    df = df.copy().sort_values("flight_date")
...
def add_rolling_rate_feature(df: pd.DataFrame, group_cols: list[str], flag_col: str, out_col: str, windows=(30, 90)) -> pd.DataFrame:
    """
    Rolling rate for a binary flag (mean), shifted(1) to avoid leakage.
    """
    df = df.copy().sort_values("flight_date")
...
# IMPORTANT:
# We read the full column list you provided.
flights_usecols = [
...
# Airline name / airport state enrich
...
# --------------------------
# IMPORTANT:
# For a pre-flight model, we typically exclude cancelled/diverted flights.
# We'll keep your approach.
...
# Scheduled times -> time buckets (pre-flight safe)
...
# Congestion (counts by scheduled hour, still pre-flight)
flights_model = add_congestion_features(flights_model)

# ---- V6 encodings (no leakage: shifted rolling over delayed) ----
...
# ---- V7 Operational signals (historical, shifted) ----
# Key idea: use historical patterns of operational delays as proxies for system state.
# NOTE: We do NOT use the current flight's realized delays.
# We only use them to build historical aggregates with shift(1).

# Flags for causes (historical proxy)
...
# (A) Tail-number cascade signals
# Rolling mean of DEPARTURE_DELAY by tail (shifted)
if "TAIL_NUMBER" in flights_model.columns:
    flights_model = add_rolling_mean_feature(
        flights_model,
        group_cols=["TAIL_NUMBER"],
        value_col="DEPARTURE_DELAY",
        out_col="tail_dep_delay_mean",
        windows=(3, 5, 10)
    )
    # Rolling delayed-rate by tail
    flights_model = add_rolling_target_rate(
        flights_model,
        group_cols=["TAIL_NUMBER"],
        out_col="tail_delayed_rate",
        windows=(5, 10)
    )

# (B) Airport operational signals (origin)
flights_model = add_rolling_mean_feature(
    flights_model,
    group_cols=["ORIGIN_AIRPORT"],
    value_col="DEPARTURE_DELAY",
    out_col="origin_dep_delay_mean",
    windows=(30, 90)
)
flights_model = add_rolling_rate_feature(
    flights_model,
    group_cols=["ORIGIN_AIRPORT"],
    flag_col="flag_weather_delay",
    out_col="origin_weather_rate",
    windows=(90, 180)
)
flights_model = add_rolling_rate_feature(
    flights_model,
    group_cols=["ORIGIN_AIRPORT"],
    flag_col="flag_system_delay",
    out_col="origin_system_rate",
    windows=(90, 180)
)
flights_model = add_rolling_rate_feature(
    flights_model,
    group_cols=["ORIGIN_AIRPORT"],
    flag_col="flag_late_aircraft",
    out_col="origin_late_aircraft_rate",
    windows=(90, 180)
)

# By origin-hour (scheduled departure hour)
flights_model = add_rolling_mean_feature(
    flights_model,
    group_cols=["ORIGIN_AIRPORT", "sched_dep_hour"],
    value_col="DEPARTURE_DELAY",
    out_col="origin_hour_dep_delay_mean",
    windows=(30,)
)

# (C) Route operational signals
flights_model = add_rolling_mean_feature(
    flights_model,
    group_cols=["ROUTE"],
    value_col="DEPARTURE_DELAY",
    out_col="route_dep_delay_mean",
    windows=(30,)
)
flights_model = add_rolling_target_rate(
    flights_model,
    group_cols=["ROUTE"],
    out_col="route_delayed_rate",
    windows=(30,)
)
...
proba_val_adj  = prior_shift_adjust(proba_val,  pi_source=pi_val,  pi_target=pi_test)
proba_test_adj = prior_shift_adjust(proba_test, pi_source=pi_val,  pi_target=pi_test)

# --------------------------
# Prints (clean + consistent)
# --------------------------
print("\n==============================")
...
# TEST @ 0.50 (calibrated + prior-shift)
pred_05_adj = (proba_test_adj >= 0.5).astype(int)
auc_adj = roc_auc_score(y_test, proba_test_adj)  # invariant-ish but ok to print
...
# --------------------------
# Threshold selection on VAL_ADJ (constraint)
# --------------------------
print("\n==============================")
...
# Save artifacts
# --------------------------
model_path = OUTPUT_DIR / "catboost_model_v7.cbm"
cb.save_model(str(model_path))
print(f"\n💾 Saved CatBoost model to: {model_path}")

platt_path = OUTPUT_DIR / "platt_calibrator_v7.joblib"
joblib.dump(platt, platt_path)
print(f"💾 Saved Platt calibrator to: {platt_path}")

thr_txt = OUTPUT_DIR / f"best_threshold_v7_prec{int(MIN_PRECISION_VAL*100)}.txt"
